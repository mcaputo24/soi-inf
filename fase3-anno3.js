...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    for (const esperienza of esperienze) {
      await addDoc(collection(db, 'fase3-studente-anno3'), {
        ...baseData,
        esperienza
      });
    }
  ...
  pdfBtn.addEventListener('click', async () => {
    ...
    const q = query(collection(db, 'fase3-studente-anno3'),
      where('nome', '==', nome),
    ...
    snapshot.forEach(docSnap => {
      ...
      doc.text(`💡 Utilità: ${e.utilita || '—'}`, 10, y); y += 8;
    ...
    doc.save(`esperienze_${cognome}_${nome}_anno3.pdf`);
  });
});
